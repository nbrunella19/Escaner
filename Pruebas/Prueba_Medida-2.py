...
def main():
    ...
    try:
        ...
        medida.bridge.standby()                         # Asegura que no esté midiendo
        medida.bridge.local_unlock()                    # Quita local lockout
        medida.bridge.send_rx_value(Rs)                 # Configura Rx
        medida.bridge.set_primary_current(Ix)           #Enciende la corriente Ix
        medida.bridge.set_delay_seconds(t)
        medida.bridge.set_num_measurements(n_medidas)
        medida.bridge.set_num_statistics(n_stats)
